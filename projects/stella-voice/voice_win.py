#!/usr/bin/env python3
"""
Stella voice exchange — Windows version
...
SOUL_FILE = r"\\wsl.localhost\Ubuntu\home\aurellian\nanoclaw\groups\stella\CLAUDE.md"
...
def tag_for_speech(text: str, context: list, client) -> str:
    ...
    try:
        ...
        print(f"\n[tagger error: {e}]", file=sys.stderr)
        return text
...
def transcribe(audio: np.ndarray) -> str:
    ...
    segments, _ = _whisper.transcribe(tmp_path, language="en", vad_filter=True)
...
def stream_and_speak(client, history, system_prompt):
    ...
    def tag_and_synthesise(text):
        tagged = tag_for_speech(text, history, client)
        return synthesise(tagged)

    def prefetch(text):
        try:
            prefetch_audio[0] = tag_and_synthesise(text)
        except Exception:
        ...
        for token in stream.text_stream:
            ...
            if len(parts) > 1:
                ...
                if prefetch_audio[0]:
                    ...
                    try:
                        play_audio(tag_and_synthesise(sentence))
                    except Exception as e:
                        print(f"\n[TTS error: {e}]", file=sys.stderr)
                    sentences.pop()
                    continue
